            ...
            return (
              ...
              >
                {formatter && item?.value !== undefined && item.name ? (
                  formatter(item.value, item.name, item, index) // ✅ Correct number of args
                ) : (
                  <>
                    {!hideIndicator && (
                      itemConfig?.icon ? (
...
>(({ className, hideIcon = false, payload, verticalAlign = "bottom", nameKey }, ref) => {
  ...
  return (
    ...
    >
      {payload.map((item, i) => {
        const key = `${nameKey || (item as any).dataKey || item.value || i}`
        const itemConfig = getPayloadConfigFromPayload(config, item, key)
...
function getPayloadConfigFromPayload(config: ChartConfig, payload: any, key: string) {
  ...
  const candidateKey =
    ...
    (payloadPayload && typeof payloadPayload[key] === "string" && payloadPayload[key]) ||
    key

  return config[candidateKey as keyof typeof config]
}
